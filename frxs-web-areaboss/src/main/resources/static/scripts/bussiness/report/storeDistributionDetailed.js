...
function search() {
    ...
    $('#grid').datagrid({
        url: '../StoreReport/GetStoreDistributionDetailedReport',
        title: '',                      //标题
...
function exportExcel(data) {
    ...
    function getExcelTR(trData) {
        ...
        trHtml += "<td style='height:20px' x:str=\"'" + (trData.ShopName == "合计" ? "" : trData.ShopCode) + "\">" + (trData.ShopName == "合计" ? "" : trData.ShopCode) + "</td>";
        trHtml += "<td>" + frxs.replaceCode(trData.ShopName) + "</td>";
        trHtml += "<td>" + frxs.replaceCode(trData.ProductName) + "</td>";
        trHtml += "<td x:str=\"'" + (trData.ShopName == "合计" ? "" : trData.SKU) + "\">" + (trData.ShopName == "合计" ? "" : trData.SKU) + "</td>";
        trHtml += "<td x:str=\"'" + (trData.ShopName == "合计" ? "" : trData.BarCode) + "\">" + (trData.ShopName == "合计" ? "" : trData.BarCode) + "</td>";
        //避免导出空行为0
    ...
    if (dataCode) {
        ...
        saveAs(
            new bb(
                ...
                , { type: "html/plain;charset=utf8" }
            ), "门店配送明细表导出_" + frxs.nowDateTime("yyyyMMdd") + ".xls"
        );
    }
}
