...
function initGrid() {
    $('#grid').datagrid({
        ...
        columns: [[
            ...
            { title: '条码', field: 'BarCode', width: 100, align: 'center', formatter: frxs.formatText },
...
function search() {
    ...
    $('#grid').datagrid({
        url: '../StoreReport/GetSupplierDistributionDetailedReport',
        title: '',                      //标题
...
function exportExcel(data) {
    ...
    function getExcelTR(trData) {
        ...
        trHtml += "<td style='height:20px' x:str=\"'" + (trData.VendorName == "合计" ? "" : trData.VendorCode) + "\">" + (trData.VendorName == "合计" ? "" : trData.VendorCode) + "</td>";
        trHtml += "<td>" + frxs.replaceCode(trData.VendorName) + "</td>";
        trHtml += "<td>" + frxs.replaceCode(trData.ProductName) + "</td>";
        trHtml += "<td x:str=\"'" + (trData.VendorName == "合计" ? "" : trData.SKU) + "\">" + (trData.VendorName == "合计" ? "" : trData.SKU) + "</td>";
        trHtml += "<td x:str=\"'" + (trData.VendorName == "合计" ? "" : trData.BarCode) + "\">" + (trData.VendorName == "合计" ? "" : trData.BarCode) + "</td>";
        //避免导出空行为0
    ...
    if (dataCode) {
        ...
        saveAs(
            new bb(
                ...
                , { type: "html/plain;charset=utf8" }
            ), "供应商配送明细表导出_" + frxs.nowDateTime("yyyyMMdd") + ".xls"
        );
    }
}
