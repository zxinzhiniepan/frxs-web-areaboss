        ...
        if (isValidate) {
           ...
           if (!formData.advertisementImageSrc || $.trim(formData.advertisementImageSrc).length == 0) {
            ...
            if(productId!=""){
                ...
                url = contextPath+"/products/createSingleProduct";
            }
            ...
            xsjs.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.rspCode == "success") {
                        if(data.record!=null){
                            $("#txtBaseProductID").val(data.record)
                        }
                        $("#formTable").tabs("enableTab", 1);
                        // $('#formTable').tabs('select', 1);
                        if (!formData.productId|| parseInt(formData.productId) == 0) {
                            xsjs.addTabs({
                                url: contextPath+"/productsWeb/editProducts?productId=" + data.record,
